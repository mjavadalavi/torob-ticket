...
class MrBilitOfferStore:
    """Shared bounded offer context used by the three MrBilit adapters."""
    ...
    async def get_details(self, offer_id: str) -> OfferDetails:
        ...
        if details is None:
            raise ResourceNotFoundError(
                "جزئیات این پیشنهاد در حافظهٔ جست‌وجوی مستربلیط پیدا نشد."
            )
        return details
    ...
    async def get_refund_rules(self, offer_id: str) -> RefundRules:
        ...
        if rules is None:
            raise ResourceNotFoundError(
                "قانون استرداد این پیشنهاد مستربلیط پیدا نشد."
            )
        return rules
    ...
    def build_redirect_url(self, source_offer_id: str) -> str:
        ...
        if redirect is None:
            raise ResourceNotFoundError("پیوند این پیشنهاد مستربلیط پیدا نشد.")
        return redirect
