...
class BookingOfferStore:
    """Bounded context for Booking.ir result redirects and details."""
    ...
    async def get_details(self, offer_id: str) -> OfferDetails:
        ...
        if details is None:
            raise ResourceNotFoundError("جزئیات پیشنهاد بوکینگ پیدا نشد.")
        return details

    def build_redirect_url(self, source_offer_id: str) -> str:
        redirect = self._redirects.get(source_offer_id)
        if redirect is None:
            raise ResourceNotFoundError("پیوند پیشنهاد بوکینگ پیدا نشد.")
        return redirect
