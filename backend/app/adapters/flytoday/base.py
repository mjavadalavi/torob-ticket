...
class FlyTodayOfferStore:
    """Bounded provider context shared by FlyToday mode adapters."""
    ...
    async def get_details(self, offer_id: str) -> OfferDetails:
        ...
        if details is None:
            raise ResourceNotFoundError(
                "جزئیات این پیشنهاد در حافظهٔ جست‌وجوی فلای‌تودی پیدا نشد."
            )
        return details
    ...
    def build_redirect_url(self, source_offer_id: str) -> str:
        ...
        if redirect is None:
            raise ResourceNotFoundError("پیوند این پیشنهاد فلای‌تودی پیدا نشد.")
        return redirect
